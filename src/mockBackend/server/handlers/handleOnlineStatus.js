let userOnlineStatuses = [2, 5, 8, 12, 17, 20, 24, 33, 36, 40, 41, 43, 45, 48];

const addOnlineStatus = (userId) => {
  userOnlineStatuses.push(userId);
...
const removeOnlineStatus = (userId) => {
  userOnlineStatuses = userOnlineStatuses.filter(id => id !== userId);
}
...
export const checkOnlineStatus = (userId) => {
  return userOnlineStatuses.includes(userId);
}
